...
def index(request):
    ...
    if request.method != "POST":
        ...
        if tema=="-":
            ...
            latest_question_list = Question.objects.filter(topic = tema)
    
   
    distinct_topic= Question.objects.values('topic').distinct()
    template = loader.get_template('polls/index.html')
    context = {
                'title':'Lista de preguntas de la encuesta',
                'latest_question_list': latest_question_list,
                'distinct_topic': distinct_topic,
              }
...
def choice_add(request, question_id):
        ...
        if request.method =='POST':
            ...
            if form.is_valid():
                ...
                if haycorrecto and choice.correcto:
                      return render(request, 'polls/detail.html', {
                    'question': q,
                    'error_message': "ERROR: ya hay una respuesta correcta",
                    })
        ...
        return render(request, 'polls/choice_new.html', {'title':'Pregunta:'+ q.question_text,'form': form})
